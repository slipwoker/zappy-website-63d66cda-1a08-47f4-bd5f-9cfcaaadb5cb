document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function validateField(field) {
      ...
      const label = field.getAttribute('aria-label') ||
        (field.id ? (document.querySelector('label[for="' + field.id + '"]') || {}).textContent : null) ||
        field.placeholder ||
        name ||
        'This field';

      if (isRequired && value === '') {
        showFieldError(field, label.trim() + ' is required.');
        return false;
      }
      ...
      if (value !== '') {
        ...
        if (minLength && value.length < parseInt(minLength, 10)) {
          showFieldError(field, label.trim() + ' must be at least ' + minLength + ' characters.');
          return false;
        }

        const maxLength = field.getAttribute('maxlength');
        if (maxLength && value.length > parseInt(maxLength, 10)) {
          showFieldError(field, label.trim() + ' must not exceed ' + maxLength + ' characters.');
          return false;
        }
      }
  ...
  if (animatableElements.length > 0) {
    ...
    if ('IntersectionObserver' in window) {
      ...
      let scrollAnimTicking = false;
      window.addEventListener('scroll', function () {
        if (!scrollAnimTicking) {
          window.requestAnimationFrame(function () {
            checkVisibility();
            scrollAnimTicking = false;
          });
          scrollAnimTicking = true;
        }
      }, { passive: true });
    }
  }
    ...
    "translations": {
      "en": {
        "consentModal": {
          ...
          "description": "Metuki MB uses cookies to enhance your experience, analyze site usage, and assist in our marketing efforts. You can manage your preferences anytime.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '63d66cda-1a08-47f4-bd5f-9cfcaaadb5cb',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '63d66cda-1a08-47f4-bd5f-9cfcaaadb5cb';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '63d66cda-1a08-47f4-bd5f-9cfcaaadb5cb';
